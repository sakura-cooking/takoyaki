...
- 「カット済 冷凍タコ」が便利です。
- カット済みを買う場合、タコのサイズは「タコ焼き器」の焼き玉の大きさによりますが、普通サイズの場合は2-3g 程度、大玉であれば3-4g くらいが良いかもしれません。焼き玉のサイズに対してすれすれだったり大きいと、焼きむらや玉の形が乱れやすくなり、焼くのが難しくなります。
...
・「使い捨てテーブルクロス」「新聞紙」や「ブルーシート」
  → 焼き台の下には使い捨てのシートを引いておくのが絶対よいです。（生地入れの時に だいたいこぼれるので）
  → 大規模にやるばあいは、焼き場周辺の床にもシートを引いておくことをおすすめします。

・「たこ焼きチラシ」
  → 焼き場にタコやきのチラシを張っておくと雰囲気が出るかも。
```
...
・「バケツ」またはそれに準ずるもの
  → 生地の水 x2～2.5くらいの容量が良い。
     水に材料を入れると 水6Lの場合は12Lくらいになります。攪拌する余白を考えて10～15L用がおススメ

・「泡だて器」またはそれに準ずるもの（柄が長いもの）もしくは「電動ハンドミキサー」
  → バケツの底に粉がたまりやすい（というかほぼたまる、引っかかる）。
     混ぜ終わりにはさいばしなどの細いもので、隅っこをさらうようにしてください
...
* クッキングペーパー、お手拭き
- 使いすて手袋 
  → 素手調理が気になる人場合は、手袋をして 下ごしらえする。タコを切るときなんかはあるととても楽です(臭くならない)
...
必要なものは、
```
- 「ペットボトル」：生地の水に対して**2倍以上** のサイズ。水１Ｌだったら、２Ｌペットボトルを用意しよう。
- 「じょうご(漏斗)」 ：口が大きく、角度が急で、材料がボトルにストレートに落ちる「特急じょうご」タイプがよい。なければ、紙をまいたりして自作してもよい。
- 「大容量軽量カップ」：500～1000ml程度の計量カップ。大きい方が楽だし、生地入れの時にも使える
- 「はかり」「キッチンスケール」
```

- この方式だと、事前に生地を作っておき、外出で持ち運ぶのが容易になるので、アウトドアでやるときにも便利。
- 一番の難関が **空きボトルを用意する**ことですね。
- 焼くときは、 **ペットボトルから直接入れない**ようにしましょう。 **「油跳ね」「タコ焼き器オーバーフロー」など大変危険** なので、いったん注ぎ口があるものに移し替えることをお勧めします（計量カップを転用するのがおススメです）
...
```
＜焼く＞
・「たこ焼き器」 （またはカセットコンロとたこ焼き用鉄板） **必須**
  → 最近おススメ「CB-ETK-1 [カセットガスたこ焼器 スーパー炎たこ]」火力がかなり強く、一気に焼きあがるので
     最高に使いやすい。一回目からの失敗がほぼなくなる！

・「たこ焼き用 ピック(串) 「または「竹串」
  → 鉄板によっては傷がついたり、溶けてしまうので注意。竹串ならオールマイティで代用可能。
  → 竹串を新しく買うのであれば 長めがよい（鉄板が熱いので）

・「油引き」 もしくは「キッチンペーパー＋さいばしぬりぬり」 でも代用可能。火災注意
  → 油引きは相性が悪いと使いづらい。キッチングペーパーでやっちゃうのも手
...
1. 初回は、タコ焼き器の鉄板を高温で温め、油をかなり多めに塗っておく。 羽の部分を含めまんべんなく油が塗られており、底に油が少したまっているくらい。
   煙が上がるくらい熱してなじませる。 特に初回は生地が鉄板に引っ付きやすいため、かなり多めに油を使うほうがよい。

2. たこ焼き生地を入れる前に毎回 **しっかり生地を攪拌する** 。底に生地が沈殿しています。 

3. たこ焼き器へ生地を流し込む。丸い部分からあふれさせて 羽の部分まできれいに流す (※これはタコ焼き器の特性にも左右されるので、その都度微調整する)
...
* 焼き
  ...
  * タコ焼きはついつい触りたくなりますが、**表面が焼けてないうちに触りすぎるとスライム化したまま 完成し失敗します** 。
    たこ焼きで一番重要なことは **我慢**。
